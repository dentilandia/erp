...
function paginaDetalleMes(periodo: { inicio: string; fin: string }, f: FilaDoctora): string {
  ...
  const filasOtros = f.detalleInsumos
    ...
    .join("");
  return `
  <h1>Detalle de laboratorios y aparatología — ${esc(f.doctora.nombre)}</h1>
  <p class="periodo">Período ${periodo.inicio} a ${periodo.fin}</p>

  <h2>Laboratorios (aparatos) instalados este período</h2>
  <table>
...
<style>
  ...
  .nota { color: #888; }
  tr.total td, tfoot td { font-weight: 700; background: #faf8fc; }
...
<body>
  ...
  ${paginas.map((p) => `<div class="pagina">${p}</div>`).join("\n")}
</body></html>`;
        ...
        return (
          <div key={f.doctora.id} className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              <button
                onClick={() => generarLiquidacionCompletaPDF(periodo, pctHonorario, f)}
                className="flex items-center gap-1 text-xs font-medium text-white bg-[var(--acento)] px-3 py-1.5 rounded-md"
              >
                <Download size={13} /> Generar liquidación completa (PDF)
              </button>
              <button
